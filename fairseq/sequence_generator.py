...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if eos_bbsz_idx.numel() > 0:
                ...
                expand_eos_mask = eos_mask.view(eos_mask.size(0), eos_mask.size(1), 1)
                expand_eos_mask = expand_eos_mask.expand(-1, -1, max_len+1)
            ...
            expand_active_hypos = active_hypos.view(active_hypos.size(0), active_hypos.size(1), 1)
            expand_active_hypos = expand_active_hypos.expand(active_hypos.size(0), active_hypos.size(1), max_len+1)
    ...
    def replicate_first_beam_3d(self, tensor, mask, beam_size: int):
        tensor = tensor.view(-1, beam_size, tensor.size(-2), tensor.size(-1))
        tensor[mask] = tensor[mask][:, :1, :, :]
