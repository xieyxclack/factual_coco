...
class BARTHubInterface(GeneratorHubInterface):
    ...
    def encode(
        ...
        if mask_token in sentence:
            sent_spans = sentence.split(mask_token)
            tokens = (' {0} '.format(mask_token)).join(
                [self.bpe.encode(sent_span.rstrip()) for sent_span in sent_spans]
            ).strip()
        else:
    ...
    def sample(
        self, sentences: List[str], beam: int = 1, verbose: bool = False, task_id: int = 1, **kwargs
    ) -> str:
        input = [self.encode(sentence) for sentence in sentences]
        hypos = self.generate(input, beam, verbose, task_id, **kwargs)
        return [self.decode(x["tokens"]) for x in hypos], [x["tokens"].detach().cpu().numpy() for x in hypos], [x["positional_scores"].detach().cpu().numpy() for x in hypos]
